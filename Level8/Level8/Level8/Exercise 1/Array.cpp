...
namespace Chunyu // own namespace Chunyu
{
	namespace Containers //Containers namespace 
	{
		...
		template <typename T>
		Array<T>::Array(int new_size):m_data(new T[new_size]),size(new_size)
		{
		...
		template <typename T>
		Array<T>::Array(const Array<T>& new_array):m_data(new T[new_array.size]), size(new_array.size), defaultSize(new_array.defaultSize)
		{
