...
Array::Array(): m_data( new Point[10]), size(10)
{
...
{
	m_data = new Point[new_size];  //create a new point array that allocates new_size elements and assign it to m_data
...
{
	m_data = new Point[new_array.size];//create a new point array that has the same size as new_array
...
Array& Array::operator = (const Array& new_array)
{
	...
	delete[] m_data;// delete the member element array m_data void memory leak
	//create a new array that has the same size as the array in Object Array new_array
	m_data = new Point[new_array.size];
